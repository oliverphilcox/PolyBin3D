...
class PolyBin3D():
    ...
    def generate_data(self, seed=None, Pk_input=[], output_type='real', include_pixel_window=True):
        ...
        if len(Pk_input)==0:
            ...
            Pk_grid = interp1d(Pk_input[0], Pk_input[1], bounds_error=False)(self.modk_grid)
            if len(Pk_input)>2:
                Pk_grid += legendre(2)(self.muk_grid)*interp1d(Pk_input[0], Pk_input[2], bounds_error=False)(self.modk_grid)
            if len(Pk_input)>3:
                Pk_grid += legendre(4)(self.muk_grid)*interp1d(Pk_input[0], Pk_input[3], bounds_error=False)(self.modk_grid)
